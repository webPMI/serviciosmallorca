...
import { can, canAll, canAny, getPermissionsForRole, PERMISSIONS } from "../../src/lib/permissions";
...
describe("RBAC Permission System", () => {
  describe("can()", () => {
    ...
    it("returns false for unknown roles", () => {
      expect(can("unknown_role" as any, PERMISSIONS.VIEW_DASHBOARD)).toBe(false);
    });
  });

  describe("canAll()", () => {
    it("returns true when all permissions are satisfied", () => {
      expect(canAll("user", [PERMISSIONS.VIEW_DASHBOARD, PERMISSIONS.VIEW_PROFILE])).toBe(true);
    });

    it("returns false when at least one is missing", () => {
      expect(canAll("user", [PERMISSIONS.VIEW_DASHBOARD, PERMISSIONS.MANAGE_USERS])).toBe(false);
    });
  });

  describe("canAny()", () => {
    it("returns true when at least one permission matches", () => {
      expect(canAny("guest", [PERMISSIONS.VIEW_PUBLIC, PERMISSIONS.MANAGE_USERS])).toBe(true);
    });

    it("returns false when none match", () => {
      expect(canAny("guest", [PERMISSIONS.VIEW_DASHBOARD, PERMISSIONS.MANAGE_USERS])).toBe(false);
    });
  });
