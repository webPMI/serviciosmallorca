...
(function () {
  ...
  var PAGE_NAMES = { home: '🏠 Home', login: '🔑 Login', register: '📝 Register', forgotPassword: '🔒 Forgot Password', dashboard: '📊 Dashboard' };
  ...
  function toast(msg, type) {
    ...
    if (!c) {
      ...
      c.id = '__dt_toasts';
      c.style.cssText = 'position:fixed;bottom:24px;right:24px;display:flex;flex-direction:column;gap:8px;z-index:999999;pointer-events:none;';
      document.body.appendChild(c);
    }
    var colors = { success: '#2ed573', error: '#ff4757', info: '#00d4aa', warn: '#ffa502' };
    var el = document.createElement('div');
    el.style.cssText = 'padding:10px 18px;border-radius:8px;font-size:12px;font-weight:600;font-family:"Segoe UI",system-ui,sans-serif;background:' + (colors[type] || colors.info) + ';color:' + (type === 'info' ? '#1a1a2e' : '#fff') + ';box-shadow:0 4px 20px rgba(0,0,0,.35);pointer-events:auto;max-width:340px;animation:__dtSlideIn .2s ease;';
    el.textContent = msg;
    c.appendChild(el);
  ...
  function updateBadge() {
    ...
    if (errCount > 0) { badge.textContent = errCount; badge.className = '__dt_badge'; }
    else if (wrnCount > 0) { badge.textContent = wrnCount; badge.className = '__dt_badge is-warn'; }
    else { badge.className = '__dt_badge is-hidden'; }
  }
  ...
  function renderConsole(container) {
    ...
    for (var i = 0; i < shown.length; i++) {
      ...
      html += '<div class="__dt_console_line">' +
        '<span class="__dt_console_level" style="color:' + (LEVEL_COLORS[level] || '#c0c8d8') + '">' + (LEVEL_LABELS[level] || 'LOG') + '</span>' +
        '<span class="__dt_console_msg">' + escapeHtml(msg) + '</span>' +
        '</div>';
    }
  ...
  function makeSubmenu(items) {
    ...
    items.forEach(function (item) {
      ...
      btn.addEventListener('click', function () { closeMenu(); item.fn(); });
      sub.appendChild(btn);
    });
    return sub;
  }
  ...
  function build() {
    ...
    // Restore position
    var pos = (function () { try { return JSON.parse(localStorage.getItem(POS_KEY)); } catch (e) { return null; } })();
    ...
    } else { root.style.right = '24px'; root.style.bottom = '24px'; }
    ...
    function savePos() {
      var l = parseInt(root.style.left) || 0, t = parseInt(root.style.top) || 0;
    ...
    btn.addEventListener('mouseup', function (e) {
      ...
      dragging = false;
      savePos();
      if (!moved) { e.stopPropagation(); showMenu(); }
    });

    window.addEventListener('mouseup', function () { if (dragging) { dragging = false; savePos(); } });

    // ---- MENU ----
    var menuEl = null, overlay = null, consoleEl = null, isOpen = false;

    function closeMenu() {
      if (LOG_REFRESH_INTERVAL) { clearInterval(LOG_REFRESH_INTERVAL); LOG_REFRESH_INTERVAL = null; }
    ...
    function showMenu() {
      ...
      hdr.innerHTML = '<span>🛠️ DevTools</span><span class="__dt_menu_page">' + (PAGE_NAMES[detectPage()] || '') + '</span>';
      ...
      switch (page) {
        case 'login':
          pageItems = [
            { text: '🔑 Fill Login Form', fn: function () { fillField('#email', 'test@webapp.com'); fillField('#password', 'Test1234!'); toast('✅ Login filled', 'success'); } },
            { text: '⚡ Fill & Submit', fn: function () { fillField('#email', 'admin@webapp.com'); fillField('#password', 'Admin1234!'); submitForm('#login-form'); toast('⚡ Submitted', 'success'); } },
            { text: '🧹 Reset Form', fn: function () { var f = document.querySelector('#login-form'); if (f) f.reset(); toast('🧹 Cleared', 'info'); } }
          ]; break;
        case 'register':
          pageItems = [
            { text: '📝 Fill Register', fn: function () { fillField('#name', 'Test User'); fillField('#email', 'new@webapp.com'); fillField('#password', 'StrongP@ss1'); fillField('#confirmPassword', 'StrongP@ss1'); toast('✅ Register filled', 'success'); } },
            { text: '🧹 Reset Form', fn: function () { var f = document.querySelector('#register-form'); if (f) f.reset(); toast('🧹 Cleared', 'info'); } }
          ]; break;
        case 'forgotPassword':
          pageItems = [
            { text: '📧 Fill Email', fn: function () { fillField('#email', 'test@webapp.com'); toast('✅ Email filled', 'success'); } },
            { text: '🧹 Reset Form', fn: function () { var f = document.querySelector('#forgot-password-form'); if (f) f.reset(); toast('🧹 Cleared', 'info'); } }
          ]; break;
        case 'dashboard':
          pageItems = [
            { text: '👤 Auth State', fn: function () { var a = window['authStore']; if (a && a.getState) { var st = a.getState(); toast('User: ' + (st.user ? st.user.email : 'null') + ' | Role: ' + st.role, 'info'); } else toast('authStore not found', 'error'); } },
            { text: '🔝 Scroll to Top', fn: function () { window.scrollTo({ top: 0, behavior: 'smooth' }); } }
          ]; break;
        case 'home':
          pageItems = [
            { text: '🔽 Scroll to Features', fn: function () { var el = document.querySelector('#features'); if (el) el.scrollIntoView({ behavior: 'smooth' }); } },
            { text: '🔝 Scroll to Top', fn: function () { window.scrollTo({ top: 0, behavior: 'smooth' }); } }
          ]; break;
      }
      ...
      pageItems.forEach(function (pi) { menuEl.appendChild(makeItem(pi.text, pi.fn)); });

      // ---- Separator ----
      var sep1 = document.createElement('div'); sep1.className = '__dt_menu_div'; menuEl.appendChild(sep1);

      // ---- Section 2: QUICK NAV (hover submenu) ----
      var qnItems = [];
      if (page !== 'home') qnItems.push({ text: '🏠 Home', fn: function () { location.href = './'; } });
      if (page !== 'login') qnItems.push({ text: '🔑 Login', fn: function () { location.href = './login'; } });
      if (page !== 'register') qnItems.push({ text: '📝 Register', fn: function () { location.href = './register'; } });
      if (page !== 'dashboard') qnItems.push({ text: '📊 Dashboard', fn: function () { location.href = './dashboard'; } });
      qnItems.push({ text: '🔒 Forgot Password', fn: function () { location.href = './forgot-password'; } });
      menuEl.appendChild(makeItem('🧭 Quick Nav', function () { }, { submenu: makeSubmenu(qnItems) }));

      // ---- Section 3: DEVTOOLS (hover submenu) ----
      var dtItems = [
        { text: '🎨 Toggle Dark/Light', fn: function () { var t = getTheme(); setTheme(t === 'dark' ? 'light' : 'dark'); toast('🎨 ' + (t === 'dark' ? 'Light' : 'Dark'), 'success'); } },
        {
          text: '💾 Dump localStorage', fn: function () {
            var o = {}; for (var i = 0; i < localStorage.length; i++) { var k = localStorage.key(i); if (k) o[k] = localStorage.getItem(k); }
            console.log('localStorage:\n' + JSON.stringify(o, null, 2));
            toast('📋 ' + localStorage.length + ' keys → console', 'info');
          }
        },
        { text: '🗑️ Clear localStorage', fn: function () { var n = localStorage.length; localStorage.clear(); toast('🗑️ Cleared (' + n + ' keys)', 'info'); } },
        { text: '🔄 Reload Page', fn: function () { window.location.reload(); } },
        { text: '🔴 Disable DevTools', fn: function () { document.cookie = D + '=;path=/;expires=Thu,01 Jan 1970 00:00:00 GMT;SameSite=Lax'; window.location.reload(); } }
      ];
      menuEl.appendChild(makeItem('🛠️ DevTools', function () { }, { submenu: makeSubmenu(dtItems) }));

      // ---- Separator ----
      var sep2 = document.createElement('div'); sep2.className = '__dt_menu_div'; menuEl.appendChild(sep2);
      ...
      var errCount = logger ? logger.getByLevel('error').length : 0;
      logHdr.innerHTML = '<span>📋 Live Console (' + logCount + ' entries' + (errCount ? ', ' + errCount + ' err' : '') + ')</span>' +
      ...
      LOG_REFRESH_INTERVAL = setInterval(function () {
        if (consoleEl && isOpen) {
          ...
          if (prevCount !== newCount && logHdr) {
            ...
            logHdr.innerHTML = '<span>📋 Live Console (' + newCount + ' entries' + (eCount ? ', ' + eCount + ' err' : '') + ')</span>' +
              '<span style="cursor:pointer;font-size:11px;color:#00d4aa" id="__dt_clr_logs">Clear</span>';
  ...
  document.addEventListener('keydown', function (e) {
    ...
    var handled = true;
    switch (e.key) {
      case 'D': e.preventDefault(); document.cookie = D + '=;path=/;expires=Thu,01 Jan 1970 00:00:00 GMT;SameSite=Lax'; window.location.reload(); break;
      case 'H': e.preventDefault(); location.href = './'; break;
      case 'L': e.preventDefault(); location.href = './login'; break;
      case 'E': e.preventDefault(); location.href = './register'; break;
      case 'B': e.preventDefault(); location.href = './dashboard'; break;
      case 'T': e.preventDefault(); var cur = getTheme(); setTheme(cur === 'dark' ? 'light' : 'dark'); toast('🎨 ' + (cur === 'dark' ? 'Light' : 'Dark'), 'success'); break;
      case '1': e.preventDefault(); setTheme('light'); toast('🎨 Green Light', 'success'); break;
      case '2': e.preventDefault(); setTheme('dark'); toast('🎨 Green Dark', 'success'); break;
      case '3': e.preventDefault(); setTheme('golden'); toast('🎨 Gold Light', 'success'); break;
      case '4': e.preventDefault(); setTheme('golden-dark'); toast('🎨 Gold Dark', 'success'); break;
      case 'F': e.preventDefault(); var p = detectPage();
        if (p === 'login') { fillField('#email', 'test@webapp.com'); fillField('#password', 'Test1234!'); toast('✅ Login filled', 'success'); }
        else if (p === 'register') { fillField('#name', 'Test User'); fillField('#email', 'new@webapp.com'); fillField('#password', 'StrongP@ss1'); fillField('#confirmPassword', 'StrongP@ss1'); toast('✅ Register filled', 'success'); }
        else if (p === 'forgotPassword') { fillField('#email', 'test@webapp.com'); toast('✅ Email filled', 'success'); }
        else handled = false;
        break;
      case 'R': e.preventDefault(); window.location.reload(); break;
      case 'K': e.preventDefault(); toast('🛠️ Use floating button menu for DevTools', 'info'); break;
      default: handled = false;
    }
    if (handled) console.log('%c⌨️ Ctrl+Shift+' + e.key + '%c executed', 'color:#00d4aa', '');
  });

  console.log('%c🛠️ DevTools active %c| %c🛠️ for menu %c| %cCtrl+Shift+D to disable',
