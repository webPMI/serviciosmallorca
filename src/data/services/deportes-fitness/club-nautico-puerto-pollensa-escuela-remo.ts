import type { ServiceItem } from "../types.ts";

export const club_nautico_puerto_pollensa_escuela_remo: ServiceItem = {
  id: "club-nautico-puerto-pollensa-escuela-remo",
  slug: "club-nautico-puerto-pollensa-escuela-remo",
  name: "Real Club Náutico Puerto Pollensa & Escuela de Remo",
  category: "deportes-fitness",
  sectorId: "deportes-fitness",
  subcategories: ["club-nautico-pollensa", "escuela-remo-mallorca", "vela-amarraderos-norte"],
  zone: "alcudia-pollensa",
  address: "Carrer de la Verge del Carme, 1, 07470 Port de Pollença",
  addressAccuracy: "verified_manual",
  coordinates: {
    lat: 39.9078,
    lng: 3.0845,
  },
  coordinatesAccuracy: "verified_manual",
  rating: 4.9,
  ratingSource: "verified_manual",
  reviewCount: 520,
  reviewCountSource: "verified_manual",
  priceRange: "€€€",
  verified: true,
  featured: false,
  status: "open",
  tags: ["zona:alcudia-pollensa", "product:vela-ligera", "mod:cita-previa"],
  phone: "+34 971 864 635",
  whatsapp: "+34 971 864 635",
  email: "info@cnportpollenca.com",
  website: "https://cnportpollenca.com",
  schedule: "Diario: 08:00 - 21:00",
  image: "/images/services/club-nautico-puerto-pollensa-escuela-remo.jpg",
  gallery: ["/images/services/club-nautico-puerto-pollensa-escuela-remo.jpg"],
  googleMapsUrl:
    "https://www.google.com/maps/search/?api=1&query=Real%20Club%20N%C3%A1utico%20Puerto%20Pollensa%20%26%20Escuela%20de%20Remo%20Carrer%20de%20la%20Verge%20del%20Carme%2C%201%2C%2007470%20Port%20de%20Pollen%C3%A7a",
  appleMapsUrl:
    "https://maps.apple.com/?q=Real%20Club%20N%C3%A1utico%20Puerto%20Pollensa%20%26%20Escuela%20de%20Remo%20Carrer%20de%20la%20Verge%20del%20Carme%2C%201%2C%2007470%20Port%20de%20Pollen%C3%A7a",
  bingMapsUrl:
    "https://bing.com/maps?q=Real%20Club%20N%C3%A1utico%20Puerto%20Pollensa%20%26%20Escuela%20de%20Remo%20Carrer%20de%20la%20Verge%20del%20Carme%2C%201%2C%2007470%20Port%20de%20Pollen%C3%A7a",
  shortDescription: {
    es: "Club náutico histórico fundado en 1961 en la bahía de Pollença: escuela de remo olímpico y de mar, cursos de vela ligera, 400 amarres y restaurante panorámico.",
    en: "Historic nautical club founded in 1961 in Pollença Bay: coastal and Olympic rowing school, dinghy sailing academy, 400 berths, and oceanview restaurant.",
    ca: "Club nàutic històric fundat el 1961 a la badia de Pollença: escola de rem olímpic i de mar, cursos de vela i 400 amarraments.",
    de: "Historischer Segelclub von 1961 in der Bucht von Pollença: Küsten- und Olympische Ruderschule, Segelakademie, 400 Liegeplätze und Panoramarestaurant.",
  },
  fullDescription: {
    es: "El RCN Port de Pollença es un referente de los deportes náuticos en el norte de Mallorca. Ubicado en las aguas protegidas de la bahía, ofrece instalaciones de primer nivel para la práctica del remo, la vela, el kayak y el paddle surf, además de organizar regatas internacionales.",
    en: "RCN Port de Pollença is north Mallorca's premier water sports hub. Set in sheltered Pollença Bay, it offers world-class facilities for rowing, sailing, kayaking, and SUP, hosting international regattas.",
    ca: "Referent dels esports nàutics al nord de Mallorca. A les aigües protegides de la badia, ofereix rem, vela, caiac i paddle surf.",
    de: "Nautisches Sportzentrum im Norden Mallorcas in der geschützten Bucht von Pollença: Rudern, Segeln, Kajak und Stand-up-Paddling sowie internationale Regatten.",
  },
  highlights: {
    es: [
      "Club náutico con más de 60 años de tradición en la Bahía de Pollença",
      "Única escuela de remo de mar certificada por la Federación Balear de Remo en el norte",
      "400 amarres para embarcaciones de hasta 25 metros de eslora",
      "Restaurante del club náutico con amplia terraza con vistas al Cap de Formentor",
    ],
    en: [
      "Nautical club with 60+ years of heritage in Pollença Bay",
      "Only Balearic Rowing Federation certified coastal rowing school in the north",
      "400 berths accommodating vessels up to 25 metres LOA",
      "Clubhouse restaurant featuring a spacious terrace facing Cap de Formentor",
    ],
    ca: [
      "Club nàutic amb més de 60 anys de tradició",
      "Única escola de rem de mar certificada al nord de Mallorca",
      "400 amarraments per a embarcacions de fins a 25 metres",
      "Restaurant amb terrassa i vistes al Cap de Formentor",
    ],
    de: [
      "Segelclub mit über 60 Jahren Tradition in der Bucht von Pollença",
      "Einzige vom Balearischen Ruderverband zertifizierte Küstenruderschule im Norden",
      "400 Liegeplätze für Yachten bis 25 Meter Länge",
      "Clubhaus-Restaurant mit großer Panoramaterrasse zum Cap Formentor",
    ],
  },
  servicesProvided: {
    es: [
      "Cursos de remo de mar y olímpico para niños, jóvenes y adultos durante todo el año",
      "Escuela de vela ligera en clases Optimist, Laser y RS Feva",
      "Alquiler de piraguas, kayaks de mar y tablas de stand-up paddle surf (SUP)",
      "Alquiler de amarres diarios, mensuales y de temporada para socios y visitantes",
    ],
    en: [
      "Year-round coastal and Olympic rowing courses for kids, teens, and adults",
      "Dinghy sailing school covering Optimist, Laser, and RS Feva classes",
      "Sea kayak, canoe, and stand-up paddleboard (SUP) rentals",
      "Daily, monthly, and seasonal boat berth rentals for members and visiting yachts",
    ],
    ca: [
      "Cursos de rem de mar i olímpic durant tot l'any",
      "Escola de vela lleugera en Optimist i Làser",
      "Lloguer de caiacs de mar i taules de SUP",
      "Lloguer d'amarraments diaris i de temporada",
    ],
    de: [
      "Ganzjähriges Küsten- und Olympisches Rudertraining für Kinder und Erwachsene",
      "Jollensegelkurse in Optimist, Laser und RS Feva",
      "Verleih von Seekajaks, Kanus und Stand-Up-Paddle-Boards (SUP)",
      "Tages-, Monats- und Saisonliegeplätze für Mitglieder und Gastlieger",
    ],
  },
  reputationBreakdown: {
    googleMaps: {
      rating: 4.9,
      reviewCount: 520,
      url: "https://www.google.com/maps/search/?api=1&query=Real%20Club%20N%C3%A1utico%20Puerto%20Pollensa%20%26%20Escuela%20de%20Remo%20Carrer%20de%20la%20Verge%20del%20Carme%2C%201%2C%2007470%20Port%20de%20Pollen%C3%A7a",
    },
  },
  createdAt: "2026-08-30",
  lastUpdatedAt: "2026-08-30",
};
