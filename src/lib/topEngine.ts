...
export function getTopServicesByCategory(
  ...
  return ranked.map((item, idx) => ({
    ...item,
    rank: idx + 1,
    badgeLabel: idx === 0 ? "🥇 Top #1 Sector" : idx === 1 ? "🥈 Top #2" : idx === 2 ? "🥉 Top #3" : `#${idx + 1}`,
  }));
}
 ...
 */
export function getTopRankedServices(limit = 10): RankedService[] {
  ...
  return ranked.map((item, idx) => ({
    ...item,
    rank: idx + 1,
    badgeLabel: idx === 0 ? "👑 Top #1 Mallorca" : `#${idx + 1} Destacado`,
  }));
}
